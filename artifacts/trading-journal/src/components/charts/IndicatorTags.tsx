...
const IndicatorTags = memo(function IndicatorTags({ topOffset = 20 }: { topOffset?: number }) {
  ...
  return (
    <div style={{ position: "absolute", top: topOffset, left: 8, zIndex: 20, display: "flex", flexDirection: "column", gap: 3, pointerEvents: "all" }}>
...
function IndicatorTag({ indicator, onToggleVisible, onDelete }: { indicator: AppliedIndicator; onToggleVisible: () => void; onDelete: () => void; }) {
  ...
  const handleMore = () => {
    ...
    if (rect) { setMenuAnchor({ top: rect.bottom + 4, left: rect.left }); setShowMore(true); }
  };

  const controlsVisible = isTouchDevice || hovered;

  return <>
    <div style={{ display: "flex", alignItems: "center", gap: 3, minHeight: 34, background: hovered ? "rgba(0,0,0,0.64)" : "rgba(0,0,0,0.48)", border: "1px solid rgba(0,0,0,0.18)", borderRadius: 7, backdropFilter: "blur(8px)", paddingLeft: 9, paddingRight: 5, userSelect: "none" }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      ...
      {controlsVisible && <div style={{ display: "flex", alignItems: "center", marginLeft: 6 }}>
        ...
        <IconBtn title="Settings" onClick={() => setShowSettings(true)}><Settings style={{ width: 16, height: 16 }} /></IconBtn>
    ...
    {showSettings && <IndicatorSettingsPanel indicator={indicator} onClose={() => setShowSettings(false)} />}
  </>;
}
