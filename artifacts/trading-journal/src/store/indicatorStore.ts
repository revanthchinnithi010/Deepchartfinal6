...
function sourceExpr(s:unknown){switch(String(s??"close").toLowerCase()){case"open":return"open";case"high":return"high";case"low":return"low";case"hl2":return"hl2";case"hlc3":return"hlc3";case"ohlc4":return"ohlc4";default:return"close"}}
...
function loadApplied():AppliedIndicator[]{try{const s=localStorage.getItem(LS_KEY);if(s){const parsed=toArray<AppliedIndicator>(JSON.parse(s),"indicatorStore.loadApplied:localStorage");return parsed.filter(i=>["EMA","SMA","RSI","VWAP","SUPERTREND","CUSTOM"].includes(i?.type)).map(normalize)}return migrateOld()}catch{return[]}}
...
export const useIndicatorStore=create<IndicatorStoreState>((set,get)=>({
 ...
 addIndicator:(type,label,overrides={})=>{const settings={...(DEFAULT_SETTINGS[type]??{}),...(overrides.settings??{})};const period=DEFAULT_SETTINGS[type]?.period as number|undefined;const ind=normalize({id:genId(),type,label:period?`${label} (${period})`:label,visible:true,color:DEFAULT_COLORS[type],lineWidth:1,lineStyle:"solid",opacity:1,settings,visibleTimeframes:[],...overrides});const next=[...get().appliedIndicators,ind];saveApplied(next);set({appliedIndicators:next})},
 ...
 updateIndicator:(id,changes)=>{const next=get().appliedIndicators.map(i=>{if(i.id!==id)return i;const settings={...i.settings,...(changes.settings??{})};return normalize({...i,...changes,settings} as AppliedIndicator)});saveApplied(next);set({appliedIndicators:next})},
 duplicateIndicator:id=>{const orig=get().appliedIndicators.find(i=>i.id===id);if(!orig)return;const next=[...get().appliedIndicators,{...normalize(orig),id:genId(),label:`${orig.label} (copy)`}];saveApplied(next);set({appliedIndicators:next})},
}));
