...
if (typeof window !== "undefined") {
  ...
  window.setInterval(() => { if (document.querySelector("[data-drawing-popup], [role=\"dialog\"]")) { __hideDrawingTooltip(); return; } const state=useDrawingStore.getState(); const selected=state.selectedDrawingId==null?null:state.drawings.find(d=>d.id===state.selectedDrawingId); if(selected && ["trendline","extended","ray","rect"].includes(selected.toolType)) __showDrawingTooltip(selected); else __hideDrawingTooltip(); }, 150);
}
`;
  src = src.replace(marker, marker + block);
}

// Repair older builds that already contain the tooltip listener.
...
src = src.replace(/z-index:2147483647/g, "z-index:9999");
...
console.log("[drawing-fix] Tooltip forced visible with deterministic refresh and high stacking order; hidden only while alert/modal UI is open.");
